...
impl VariationalEncoder {
    fn new(vs: VarBuilder, input_dim: usize, hidden_dims: &[usize], latent_dim: usize) -> CandleResult<Self> {
        let mut shared_layers = Vec::new();
        let mut prev_dim = input_dim;
        
        // Create shared hidden layers
        for (i, &hidden_dim) in hidden_dims.iter().enumerate() {
            let layer = linear(prev_dim, hidden_dim, vs.pp(&format!("encoder.shared.{}", i)))?;
        ...
        Ok(Self { shared_layers, mean_layer, logvar_layer })
    }
...
impl VariationalDecoder {
    fn new(vs: VarBuilder, latent_dim: usize, hidden_dims: &[usize], output_dim: usize) -> CandleResult<Self> {
        let mut layers = Vec::new();
        let mut prev_dim = latent_dim;
        
        // Create hidden layers (reverse of encoder)
        for (i, &hidden_dim) in hidden_dims.iter().rev().enumerate() {
            let layer = linear(prev_dim, hidden_dim, vs.pp(&format!("decoder.hidden.{}", i)))?;
            layers.push(layer);
            prev_dim = hidden_dim;
        }
...
impl VariationalAutoencoder {
    ...
    pub fn init_network(&mut self, hidden_dims: &[usize]) -> Result<(), String> {
        ...
        let encoder = VariationalEncoder::new(vs.clone(), self.input_dim, hidden_dims, self.latent_dim)
            .map_err(|e| format!("Failed to create variational encoder: {}", e))?;
        let decoder = VariationalDecoder::new(vs, self.latent_dim, hidden_dims, self.input_dim)
            .map_err(|e| format!("Failed to create variational decoder: {}", e))?;
    ...
    pub fn forward(&self, x: &Tensor) -> CandleResult<(Tensor, Tensor, Tensor)> {
        let encoder = self.encoder.as_ref().ok_or_else(|| {
            candle_core::Error::Msg("Encoder not initialized".into())
        })?;
        let decoder = self.decoder.as_ref().ok_or_else(|| {
            candle_core::Error::Msg("Decoder not initialized".into())
        })?;
    ...
    pub fn train_step(&mut self, vectors: &Array2<f32>) -> Result<f32, String> {
        ...
        // Forward pass
        let (reconstruction, mean, logvar) = self.forward(&batch_tensor)
    ...
    pub fn encode(&self, vectors: &Array2<f32>) -> Result<(Array2<f32>, Array2<f32>), String> {
        ...
        let (mean, logvar) = encoder.encode(&input_tensor)
    ...
    pub fn sample_latent(&self, mean: &Array2<f32>, logvar: &Array2<f32>) -> Result<Array2<f32>, String> {
        ...
        let z = encoder.reparameterize(&mean_tensor, &logvar_tensor)
            .map_err(|e| format!("Reparameterization failed: {}", e))?;
    ...
    pub fn decode(&self, latent_vectors: &Array2<f32>) -> Result<Array2<f32>, String> {
        ...
        let output = decoder.forward(&latent_tensor)
    ...
    pub fn evaluate_reconstruction(&self, vectors: &Array2<f32>) -> Result<HashMap<String, f32>, String> {
        ...
        // MSE
        let mse = vectors.iter().zip(reconstruction_array.iter())
            .map(|(a, b)| (a - b).powi(2))
        ...
        // Mean absolute error
        let mae = vectors.iter().zip(reconstruction_array.iter())
            .map(|(a, b)| (a - b).abs())
            .sum::<f32>() / (vectors.len() as f32);
    ...
    fn array_to_tensor(&self, array: &Array2<f32>) -> Result<Tensor, String> {
        ...
        Tensor::from_vec(data, (shape[0], shape[1]), &self.device)
            .map_err(|e| format!("Array to tensor conversion failed: {}", e))
    }
    ...
    fn tensor_to_array(&self, tensor: &Tensor) -> Result<Array2<f32>, String> {
        ...
        let data = tensor.to_vec2::<f32>()
            .map_err(|e| format!("Tensor to array conversion failed: {}", e))?;
