...
impl OpeningBook {
    ...
    fn add_deeper_openings(&mut self) {
        ...
        if let Ok(board) = Board::from_str("r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4") {
            let moves = vec![
        ...
        if let Ok(board) = Board::from_str("rnbqkbnr/pppp1ppp/4p3/8/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 2") {
            let moves = vec![
        ...
        if let Ok(board) = Board::from_str("rnbqkbnr/pp1ppppp/2p5/8/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 2") {
            let moves = vec![
        ...
        if let Ok(board) = Board::from_str("rnbqkbnr/pppp1ppp/8/4p3/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
        ...
        if let Ok(board) = Board::from_str("rnbqkb1r/pppppp1p/5np1/8/2PP4/8/PP2PPPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
        ...
        if let Ok(board) = Board::from_str("rnbqk2r/pppp1ppp/4pn2/8/1bPP4/2N5/PP2PPPP/R1BQKBNR w KQkq - 2 4") {
            let moves = vec![
    ...
    fn add_comprehensive_openings(&mut self) {
        ...
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("e4d5").unwrap(), 1.0),   // Main line
            ];
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Scandinavian Defense".to_string(), Some("B01".to_string()));
        }
        
        // ALEKHINE'S DEFENSE
        if let Ok(board) = Board::from_str("rnbqkb1r/pppppppp/5n2/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 1 2") {
            let moves = vec![
                (ChessMove::from_str("e4e5").unwrap(), 1.0),   // Chase the knight
                (ChessMove::from_str("b1c3").unwrap(), 0.8),   // Four Knights variation
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Alekhine's Defense".to_string(), Some("B02".to_string()));
        }
        
        // PIRC DEFENSE
        if let Ok(board) = Board::from_str("rnbqkb1r/ppp1pppp/3p1n2/8/3PP3/8/PPP2PPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
                (ChessMove::from_str("b1c3").unwrap(), 1.0),   // Classical setup
                (ChessMove::from_str("f2f4").unwrap(), 0.8),   // Austrian Attack
                (ChessMove::from_str("g1f3").unwrap(), 0.9),   // Quiet development
            ];
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Pirc Defense".to_string(), Some("B07".to_string()));
        }
        
        // SICILIAN NAJDORF
        if let Ok(board) = Board::from_str("rnbqkb1r/1pp1pppp/p4n2/3p4/3P4/5N2/PPP1PPPP/RNBQKB1R w KQkq - 0 4") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Sicilian Najdorf".to_string(), Some("B90".to_string()));
        }
        
        // SICILIAN DRAGON
        if let Ok(board) = Board::from_str("rnbqk2r/pp2ppbp/3p1np1/8/3P4/2N2N2/PPP1PPPP/R1BQKB1R w KQkq - 0 6") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Sicilian Dragon".to_string(), Some("B70".to_string()));
        }
        
        // RUY LOPEZ - MORPHY DEFENSE
        if let Ok(board) = Board::from_str("r1bqkbnr/1ppp1ppp/p1n5/1B2p3/4P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 0 4") {
            let moves = vec![
                (ChessMove::from_str("b5a4").unwrap(), 1.0),   // Spanish main line
                (ChessMove::from_str("b5c6").unwrap(), 0.6),   // Exchange variation
            ];
            let _ = self.add_opening(&board.to_string(), 0.4, moves, "Ruy Lopez - Morphy Defense".to_string(), Some("C78".to_string()));
        }
        
        // RUY LOPEZ CLOSED
        if let Ok(board) = Board::from_str("r1bqk1nr/pppp1ppp/2n5/1Bb1p3/4P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Ruy Lopez - Closed System".to_string(), Some("C84".to_string()));
        }
        ...
        if let Ok(board) = Board::from_str("r1bqkb1r/pppp1ppp/2n2n2/4p3/2B1P3/5N2/PPPP1PPP/RNBQK2R w KQkq - 4 4") {
            ...
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Italian Game - Two Knights".to_string(), Some("C55".to_string()));
        }
        
        // KING'S GAMBIT
        if let Ok(board) = Board::from_str("rnbqkbnr/pppp1ppp/8/4p3/4PP2/8/PPPP2PP/RNBQKBNR b KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("e5f4").unwrap(), 1.0),   // Accept the gambit
                (ChessMove::from_str("f8c5").unwrap(), 0.7),   // Decline with Bc5
            ];
            let _ = self.add_opening(&board.to_string(), 0.0, moves, "King's Gambit".to_string(), Some("C30".to_string()));
        }
        
        // QUEEN'S GAMBIT ACCEPTED
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp1pppp/8/8/2pP4/8/PP2PPPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Queen's Gambit Accepted".to_string(), Some("D20".to_string()));
        }
        
        // QUEEN'S GAMBIT DECLINED
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp2ppp/4p3/3p4/2PP4/8/PP2PPPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
                (ChessMove::from_str("b1c3").unwrap(), 1.0),   // Classical QGD
                (ChessMove::from_str("g1f3").unwrap(), 0.9),   // Quiet development
                (ChessMove::from_str("c4d5").unwrap(), 0.6),   // Exchange variation
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Queen's Gambit Declined".to_string(), Some("D30".to_string()));
        }
        
        // LONDON SYSTEM
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp1pppp/8/3p4/3P4/5N2/PPP1PPPP/RNBQKB1R w KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("c1f4").unwrap(), 1.0),   // London system bishop
                (ChessMove::from_str("c2c4").unwrap(), 0.8),   // Transpose to QG
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "London System".to_string(), Some("D02".to_string()));
        }
        
        // CATALAN OPENING
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp2ppp/4p3/3p4/2PP4/6P1/PP2PP1P/RNBQKBNR b KQkq - 0 3") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Catalan Opening".to_string(), Some("E00".to_string()));
        }
        
        // GRUNFELD DEFENSE
        if let Ok(board) = Board::from_str("rnbqkb1r/ppp1pp1p/5np1/3p4/2PP4/2N5/PP2PPPP/R1BQKBNR w KQkq - 0 4") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Grunfeld Defense".to_string(), Some("D80".to_string()));
        }
        
        // BENONI DEFENSE
        if let Ok(board) = Board::from_str("rnbqkbnr/pp2pppp/8/2pp4/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
                (ChessMove::from_str("d4d5").unwrap(), 1.0),   // Modern Benoni
                (ChessMove::from_str("g1f3").unwrap(), 0.8),   // Quiet approach
            ];
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Benoni Defense".to_string(), Some("A60".to_string()));
        }
        
        // DUTCH DEFENSE
        if let Ok(board) = Board::from_str("rnbqkbnr/ppppp1pp/8/5p2/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Dutch Defense".to_string(), Some("A80".to_string()));
        }
        
        // VIENNA GAME
        if let Ok(board) = Board::from_str("rnbqkbnr/pppp1ppp/8/4p3/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq - 1 2") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Vienna Game".to_string(), Some("C25".to_string()));
        }
        
        // SCOTCH GAME
        if let Ok(board) = Board::from_str("rnbqkbnr/pppp1ppp/8/4p3/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("e5d4").unwrap(), 1.0),   // Main line Scotch
                (ChessMove::from_str("g8f6").unwrap(), 0.8),   // Scotch Game declined
            ];
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Scotch Game".to_string(), Some("C45".to_string()));
        }
        
        // PETROFF DEFENSE
        if let Ok(board) = Board::from_str("rnbqkb1r/pppp1ppp/5n2/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R w KQkq - 2 3") {
            let moves = vec![
                (ChessMove::from_str("f3e5").unwrap(), 1.0),   // Main line Petroff
                (ChessMove::from_str("d2d4").unwrap(), 0.8),   // Scotch Four Knights
            ];
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Petroff Defense".to_string(), Some("C42".to_string()));
        }
        
        // BIRD'S OPENING
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/5P2/8/PPPPP1PP/RNBQKBNR b KQkq - 0 1") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.0, moves, "Bird's Opening".to_string(), Some("A02".to_string()));
        }
    ...
    fn add_modern_openings(&mut self) {
        // ACCELERATED DRAGON
        if let Ok(board) = Board::from_str("rnbqkb1r/pp1ppp1p/5np1/2p5/3P4/5N2/PPP1PPPP/RNBQKB1R w KQkq - 0 4") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Sicilian - Accelerated Dragon".to_string(), Some("B35".to_string()));
        }
        
        // SICILIAN SVESHNIKOV
        if let Ok(board) = Board::from_str("r1bqkb1r/1p2pppp/p1np1n2/4p3/3PP3/2N2N2/PPP2PPP/R1BQKB1R w KQkq - 0 6") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Sicilian - Sveshnikov Variation".to_string(), Some("B33".to_string()));
        }
        
        // TROMPOWSKY ATTACK
        if let Ok(board) = Board::from_str("rnbqkb1r/pppppppp/5n2/8/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 1 2") {
            let moves = vec![
                (ChessMove::from_str("c1g5").unwrap(), 1.0),   // Trompowsky Bishop
                (ChessMove::from_str("g1f3").unwrap(), 0.8),   // Transpose to normal lines
                (ChessMove::from_str("b1c3").unwrap(), 0.7),   // Classical development
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Trompowsky Attack".to_string(), Some("A45".to_string()));
        }
        
        // TORRE ATTACK
        if let Ok(board) = Board::from_str("rnbqkb1r/ppp1pppp/5n2/3p4/3P4/5N2/PPP1PPPP/RNBQKB1R w KQkq - 0 3") {
            let moves = vec![
                (ChessMove::from_str("c1g5").unwrap(), 1.0),   // Torre Bishop
                (ChessMove::from_str("c2c4").unwrap(), 0.8),   // Transpose to QG
                (ChessMove::from_str("e2e3").unwrap(), 0.7),   // Colle system
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Torre Attack".to_string(), Some("D03".to_string()));
        }
        
        // BLACKMAR-DIEMER GAMBIT
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp1pppp/8/3p4/3PP3/8/PPP2PPP/RNBQKBNR b KQkq - 0 2") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.0, moves, "Blackmar-Diemer Gambit".to_string(), Some("D00".to_string()));
        }
        
        // SCANDINAVIAN MAIN LINE
        if let Ok(board) = Board::from_str("rnbqkbnr/ppp1pppp/8/8/3p4/8/PPP1PPPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("d1d4").unwrap(), 1.0),   // Recapture with queen
                (ChessMove::from_str("g1f3").unwrap(), 0.8),   // Develop knight first
            ];
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Scandinavian - Main Line".to_string(), Some("B01".to_string()));
        }
        
        // POLISH OPENING (SOKOLSKY)
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/1P6/8/P1PPPPPP/RNBQKBNR b KQkq - 0 1") {
            let moves = vec![
                (ChessMove::from_str("d7d5").unwrap(), 1.0),   // Central response
                (ChessMove::from_str("g8f6").unwrap(), 0.8),   // Hypermodern approach
                (ChessMove::from_str("e7e5").unwrap(), 0.7),   // Aggressive counter
            ];
            let _ = self.add_opening(&board.to_string(), 0.0, moves, "Polish Opening".to_string(), Some("A00".to_string()));
        }
        
        // NIMZOWITSCH-LARSEN ATTACK
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/8/1P6/P1PPPPPP/RNBQKBNR b KQkq - 0 1") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Nimzowitsch-Larsen Attack".to_string(), Some("A01".to_string()));
        }
        
        // RETI OPENING
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/8/5N2/PPPPPPPP/RNBQKB1R b KQkq - 1 1") {
            let moves = vec![
                (ChessMove::from_str("d7d5").unwrap(), 1.0),   // Classical response
                (ChessMove::from_str("g8f6").unwrap(), 0.9),   // Hypermodern mirror
                (ChessMove::from_str("c7c5").unwrap(), 0.6),   // English Defense
            ];
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "Reti Opening".to_string(), Some("A04".to_string()));
        }
        
        // KING'S INDIAN ATTACK (vs French setup)
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/8/5NP1/PPPPPP1P/RNBQKB1R b KQkq - 0 2") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "King's Indian Attack".to_string(), Some("A07".to_string()));
        }
        
        // BUDAPEST GAMBIT
        if let Ok(board) = Board::from_str("rnbqkb1r/pppppppp/5n2/4p3/3P4/8/PPP1PPPP/RNBQKBNR w KQkq - 1 2") {
            let moves = vec![
                (ChessMove::from_str("d4e5").unwrap(), 1.0),   // Accept the gambit
                (ChessMove::from_str("g1f3").unwrap(), 0.7),   // Decline and develop
            ];
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Budapest Gambit".to_string(), Some("A51".to_string()));
        }
        
        // BENKO GAMBIT
        if let Ok(board) = Board::from_str("rnbqkb1r/p1pppppp/5n2/1p6/2PP4/8/PP2PPPP/RNBQKBNR w KQkq - 0 3") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.3, moves, "Benko Gambit".to_string(), Some("A57".to_string()));
        }
        
        // ENGLISH - SYMMETRICAL
        if let Ok(board) = Board::from_str("rnbqkbnr/pp1ppppp/8/2p5/2P5/8/PP1PPPPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
                (ChessMove::from_str("b1c3").unwrap(), 1.0),   // Symmetrical English
                (ChessMove::from_str("g1f3").unwrap(), 0.8),   // Reti-style
                (ChessMove::from_str("g2g3").unwrap(), 0.7),   // Fianchetto
            ];
            let _ = self.add_opening(&board.to_string(), 0.1, moves, "English Opening - Symmetrical".to_string(), Some("A30".to_string()));
        }
        
        // MODERN DEFENSE
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppp1p/6p1/8/4P3/8/PPPP1PPP/RNBQKBNR w KQkq - 0 2") {
            let moves = vec![
            ...
            let _ = self.add_opening(&board.to_string(), 0.2, moves, "Modern Defense".to_string(), Some("B06".to_string()));
        }
        ...
        if let Ok(board) = Board::from_str("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq - 0 1") {
            ...
            let _ = self.add_opening(&board.to_string(), 0.0, moves, "Hippopotamus Defense".to_string(), Some("A00".to_string()));
        }
    }
