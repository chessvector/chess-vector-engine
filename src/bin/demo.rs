...
fn main() {
    ...
    let positions_with_evals = vec![
        ...
        // After 1.e4 e5 2.Nf3
        ("rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq - 1 2", 0.1),
        // Sicilian Defense
    ...
    println!("Adding {} positions to the knowledge base...", positions_with_evals.len());
    ...
    for (i, (_vector, evaluation, similarity)) in similar_positions.iter().enumerate() {
        println!("  {}. Evaluation: {:.3}, Similarity: {:.3}", 
                 i + 1, evaluation, similarity);
    }

    // Test evaluation prediction
    if let Some(predicted_eval) = engine.evaluate_position(&test_board) {
        println!("Predicted evaluation for starting position: {:.3}", predicted_eval);
    }

    // Test with a different position (after 1.e4)
    println!("\n=== Testing with 1.e4 position ===");
    let e4_board = Board::from_str("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1").expect("Valid FEN");
    ...
    let board2 = Board::from_str("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1").expect("Valid FEN");
    let board3 = Board::from_str("rnbqkbnr/pp1ppppp/8/2p5/4P3/8/PPPP1PPP/RNBQKBNR w KQkq c6 0 2").expect("Valid FEN");
    ...
    println!("Similarity between starting position and 1.e4: {:.3}", sim_1_2);
    println!("Similarity between starting position and Sicilian: {:.3}", sim_1_3);
    ...
    println!("Starting position vector (first 10 elements): {:?}", 
             &start_vector.as_slice().unwrap()[0..10]);
    
    let e4_vector = engine.encode_position(&board2);
    println!("1.e4 position vector (first 10 elements): {:?}", 
