...
use chess_vector_engine::license::{LicenseKey, LicenseVerifier, LicenseError, current_timestamp};
use tokio;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match engine.check_licensed_feature("basic_position_encoding").await {
    ...
    let all_features = [
        "basic_position_encoding", "similarity_search", "opening_book",
        "gpu_acceleration", "ultra_fast_loading", "memory_mapped_files", 
        "advanced_tactical_search", "pondering", "multi_pv_analysis",
        "distributed_training", "enterprise_analytics", "custom_algorithms"
    ];
    ...
    match engine.activate_license("INVALID-999999").await {
        Ok(tier) => {
            println!("❌ Unexpected success with invalid license! Tier: {:?}", tier);
        }
        Err(e) => {
    ...
    match verifier.check_feature_license("TEST-MANUAL", "gpu_acceleration").await {
        Ok(()) => {
