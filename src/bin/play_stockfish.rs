use chess::{Board, Game, Color, ChessMove, Piece, Square, MoveGen};
...
fn convert_to_san(board: &Board, chess_move: ChessMove) -> String {
    ...
    if piece != Piece::Pawn {
        ...
        if !same_piece_moves.is_empty() {
            ...
            let need_file_disambiguation = same_piece_moves.iter()
...
impl StockfishPlayer {
    fn new(depth: u8, time_per_move: u32) -> Result<Self, Box<dyn std::error::Error>> {
        let mut child = ProcessCommand::new("stockfish")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| format!("Failed to start Stockfish. Make sure it's installed and in PATH. Error: {}", e))?;
    ...
    fn get_best_move(&mut self, board: &Board) -> Result<(ChessMove, f32), Box<dyn std::error::Error>> {
...
impl GameState {
    ...
    fn generate_pgn(&self) -> String {
        ...
        for (i, analysis) in self.move_history.iter().enumerate() {
            ...
            pgn.push_str(&format!(" {{ eval: {:.2}, loss: {:.1}cp }} ", 
    ...
    fn calculate_statistics(&self) -> (f32, f32, f32, f32) {
        let chess_vector_moves: Vec<_> = self.move_history.iter()
            .enumerate()
            .filter(|(i, _)| {
                let move_color = if i % 2 == 0 { Color::White } else { Color::Black };
                move_color == self.chess_vector_color
            })
            .map(|(_, analysis)| analysis)
            .collect();
            
        let stockfish_moves: Vec<_> = self.move_history.iter()
            .enumerate()
            .filter(|(i, _)| {
                let move_color = if i % 2 == 0 { Color::White } else { Color::Black };
                move_color != self.chess_vector_color
        ...
        let chess_vector_total_loss: f32 = chess_vector_moves.iter()
            .map(|m| m.centipawn_loss)
            .sum();
        let chess_vector_good_moves = chess_vector_moves.iter()
        ...
        let stockfish_total_loss: f32 = stockfish_moves.iter()
            .map(|m| m.centipawn_loss)
            .sum();
        let stockfish_good_moves = stockfish_moves.iter()
            .filter(|m| m.centipawn_loss < 20.0)
            .count();
        ...
        (chess_vector_total_loss, chess_vector_accuracy, stockfish_total_loss, stockfish_accuracy)
    }
}
...
fn play_game(
    ...
    println!("♟️  Chess Vector plays as: {}", if chess_vector_color == Color::White { "White" } else { "Black" });
    println!("🐟 Stockfish: depth {}, {}ms per move", stockfish_depth, time_per_move);
    println!("🤖 Chess Vector: tactical depth {}, {}ms per move\n", tactical_depth, time_per_move);
    
    let mut game_state = GameState::new(chess_vector_color);
    
    // Initialize Chess Vector Engine with premium features unlocked
    // Note: This is a premium demonstration tool requiring a license
    println!("🤖 Initializing Chess Vector Engine (Premium Demo)...");
    let mut chess_vector_engine = ChessVectorEngine::new_with_tier(1024, FeatureTier::Premium);
    ...
    println!("⚔️  Strong tactical search enabled ({} ply depth, {}ms time, 2M nodes, 8 threads)", tactical_depth, time_per_move);
    ...
    if let Err(e) = chess_vector_engine.auto_load_training_data() {
        println!("⚠️  No training data found, using tactical search fallback: {}", e);
    ...
    for lichess_path in lichess_paths {
        ...
        if std::path::Path::new(&expanded_path).exists() {
            ...
            if chess_vector_engine.is_feature_available("ultra_fast_loading") {
                ...
                match chess_vector_engine.load_lichess_puzzles_premium(&expanded_path) {
                    Ok(()) => {
                ...
                match chess_vector_engine.load_lichess_puzzles_basic(&expanded_path, 50_000) {
                    Ok(()) => {
    ...
    if chess_vector_engine.is_feature_available("gpu_acceleration") {
        println!("🚀 GPU acceleration available (Premium feature)");
    }
    if chess_vector_engine.is_feature_available("advanced_tactical_search") {
    ...
    while game_state.game.result().is_none() && move_count < 200 {
        ...
        println!("Turn: {}", if current_board.side_to_move() == Color::White { "White" } else { "Black" });
        ...
        if is_chess_vector_turn {
            ...
            let eval_method = if chess_vector_engine.get_opening_entry(&current_board).is_some() {
                "📚 Opening Book"
            } else {
                // This is a simplified check - the engine internally decides between patterns and tactical
                "🧠 Pattern Recognition + ⚔️ Tactical Search"
            };
            
            // Get current evaluation using hybrid approach
            let eval_before = chess_vector_engine.evaluate_position(&current_board).unwrap_or(0.0);
            ...
            let eval_after_raw = chess_vector_engine.evaluate_position(&temp_board).unwrap_or(0.0);
            ...
            println!("🎯 Chess Vector plays: {} | Method: {} | Confidence: {:.2} | Eval: {:.2}", 
            ...
            println!("🎯 Stockfish plays: {} (eval: {:.2})", best_move, eval_for_player);
    ...
    // Save PGN to file
    let filename = format!("chess_vector_vs_stockfish_{}.pgn", 
...
fn calculate_centipawn_losses(move_history: &mut [MoveAnalysis]) {
    for i in 0..move_history.len() {
        if i > 0 {
            ...
            let eval_diff = if i % 2 == 1 { // Odd index = White just moved (0-indexed)
                // White move: if evaluation decreased, that's bad for white
                prev_eval - current_eval
            } else { // Even index = Black just moved
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match ProcessCommand::new("stockfish").arg("--help").output() {
        Ok(_) => {},
        Err(_) => {
            eprintln!("❌ Stockfish not found! Please install Stockfish and make sure it's in your PATH.");
            eprintln!("   Download from: https://stockfishchess.org/download/");
            std::process::exit(1);
        }
    }
    
    play_game(chess_vector_color, stockfish_depth, time_per_move, tactical_depth)
}