...
fn main() {
    ...
    println!("Average encoding time: {:?} per position", encoding_time / iterations);
    println!("Encoding rate: {:.0} positions/second", 
    ...
    for size in test_sizes {
        ...
        if test_engine.is_lsh_enabled() {
            if let Some(stats) = test_engine.lsh_stats() {
                println!("  LSH Stats - Tables: {}, Hash size: {}, Avg bucket: {:.1}",
        ...
        println!("  Size: {}, Build: {:?}, Search: {:?}, Found: {} results", 
                 size, build_time, search_time, results.len());
        println!("  Search rate: {:.0} queries/second", 
