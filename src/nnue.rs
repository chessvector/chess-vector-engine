use chess::{Board, Color, Square, Piece};
...
impl NNUE {
    ...
    pub fn new(config: NNUEConfig) -> CandleResult<Self> {
        ...
        let feature_transformer = FeatureTransformer::new(vs.clone(), config.feature_size, config.hidden_size)?;
        ...
        for i in 0..config.num_hidden_layers {
            let layer = linear(prev_size, config.hidden_size, vs.pp(&format!("hidden_{}", i)))?;
    ...
    fn extract_features(&self, board: &Board) -> CandleResult<Tensor> {
        ...
        for square in chess::ALL_SQUARES {
            if let Some(piece) = board.piece_on(square) {
                ...
                let (white_idx, _black_idx) = self.get_feature_indices(piece, color, square, white_king, black_king);
    ...
    fn get_feature_indices(&self, piece: Piece, color: Color, square: Square, _white_king: Square, _black_king: Square) -> (Option<usize>, Option<usize>) {
...
impl EvalStats {
    ...
    fn add_evaluation(&mut self, eval: f32) {
        ...
        if self.count > 1 {
            let sum_sq = (self.count - 1) as f32 * self.std_dev.powi(2) + delta * (eval - self.mean);
...
impl HybridEvaluator {
    ...
    where F: Fn(&Board) -> Option<f32> + 'static {
