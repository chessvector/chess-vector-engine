...
impl LichessLoader {
    ...
    pub fn load_parallel<P: AsRef<Path>>(
            ...
            if batch.len() >= self.batch_size {
                ...
                // Accumulate results
                {
                    ...
                    *valid_guard += batch_results.len();
                    ...
                    if batch_count % 10 == 0 {
                        println!("📈 Batch {}: Processed {}k puzzles, {} valid positions",
                            batch_count, *processed_guard / 1000, *valid_guard);
                    }
                }
        ...
        println!("🚀 Speed: {:.0} puzzles/second", 
            ...
            if batch.len() >= self.batch_size {
                ...
                // Accumulate results
                {
                    ...
                    *valid_guard += batch_results.len();
                    ...
                    if batch_count % 10 == 0 {
                        println!("📈 Batch {}: Processed {}k puzzles, {} valid moves",
                            batch_count, *processed_guard / 1000, *valid_guard);
                    }
                }
    ...
    fn parse_puzzle_line(&self, line: &str) -> Result<Option<TrainingData>, Box<dyn std::error::Error>> {
        // Wrap entire parsing in panic protection to catch any chess library panics
        match std::panic::catch_unwind(|| -> Result<Option<TrainingData>, Box<dyn std::error::Error>> {
            ...
            if let Some(ref theme_filter) = self.theme_filter {
                let has_target_theme = theme_filter.iter()
    ...
    fn calculate_puzzle_evaluation(&self, rating: u32, themes: &str, board: &Board) -> f32 {
        ...
        if themes.contains("checkmate") || themes.contains("mateIn") {
            eval = if board.side_to_move() == Color::White { 500.0 } else { -500.0 };
        } else if themes.contains("fork") || themes.contains("pin") {
            eval += if board.side_to_move() == Color::White { 300.0 } else { -300.0 };
        } else if themes.contains("sacrifice") {
            eval += if board.side_to_move() == Color::White { 200.0 } else { -200.0 };
        }
    ...
    fn parse_puzzle_line_with_move(&self, line: &str) -> Result<Option<(Board, f32, ChessMove)>, Box<dyn std::error::Error>> {
        // Wrap entire parsing in panic protection to catch any chess library panics
        match std::panic::catch_unwind(|| -> Result<Option<(Board, f32, ChessMove)>, Box<dyn std::error::Error>> {
            // Use proper CSV parsing to handle quotes and commas correctly
            let mut reader = csv::ReaderBuilder::new()
                .has_headers(false)
                .from_reader(line.as_bytes());
