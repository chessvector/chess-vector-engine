...
impl TacticalSearch {
    ...
    fn parallel_root_search(&mut self, board: &Board, moves: Vec<ChessMove>, depth: u32) -> (f32, Option<ChessMove>, u32) {
    ...
    fn parallel_iterative_deepening(&mut self, board: &Board, mut moves: Vec<ChessMove>) -> (f32, Option<ChessMove>, u32) {
    ...
    fn principal_variation_search(&mut self, board: &Board, depth: u32, mut alpha: f32, mut beta: f32, maximizing: bool, moves: Vec<ChessMove>) -> (f32, Option<ChessMove>) {
        ...
        for (move_index, chess_move) in moves.into_iter().enumerate() {
            ...
            let reduction = if self.config.enable_late_move_reductions 
                ...
                && !self.is_killer_move(&chess_move) { // Don't reduce killer moves
