...
impl AutoDiscovery {
    ...
    pub fn discover_training_files<P: AsRef<Path>>(
        ...
        println!("🔍 Discovering training data files in {}...", base_path.display());
        ...
        println!("📁 Discovered {} training data files", discovered_files.len());
    ...
    pub fn cleanup_old_formats(files_to_remove: &[PathBuf], dry_run: bool) -> Result<(), Box<dyn std::error::Error>> {
        if dry_run {
            println!("🧹 DRY RUN - Would remove {} old format files:", files_to_remove.len());
        ...
        println!("🧹 Cleaning up {} old format files...", files_to_remove.len());
    ...
    fn detect_format(path: &Path) -> Result<Option<(String, FormatPriority)>, Box<dyn std::error::Error>> {
        let file_name = path.file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("");
        ...
        if file_name.contains("training") || file_name.contains("position") || file_name.contains("tactical") {
    ...
    fn verify_json_training_data(path: &Path) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        if let Ok(metadata) = std::fs::metadata(path) {
            ...
            if size == 0 || size > 10_000_000 { // Skip empty files or files > 10MB
                return Ok(false);
            }
        }
    ...
    fn verify_binary_training_data(path: &Path) -> Result<bool, Box<dyn std::error::Error>> {
        ...
        if let Ok(metadata) = std::fs::metadata(path) {
            let size = metadata.len();
            if size < 100 || size > 100_000_000 { // Skip files < 100B or > 100MB
                return Ok(false);
            }
        }
...
mod tests {
    ...
    fn test_base_name_extraction() {
        assert_eq!(AutoDiscovery::extract_base_name(Path::new("training_data.json")), "training_data");
        assert_eq!(AutoDiscovery::extract_base_name(Path::new("training_data.mmap")), "training_data");
        assert_eq!(AutoDiscovery::extract_base_name(Path::new("tactical_training_data.msgpack")), "tactical_training_data");
    }
